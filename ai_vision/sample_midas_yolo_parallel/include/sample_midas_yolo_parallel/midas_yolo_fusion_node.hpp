...
private:
  ...
  custom_msg::TensorList make_tensor_msg(const std::string & name,
      const cv::Mat & data,
      int data_type,
  ...
  // depth
  void decode_midas_depth(const custom_msg::Tensor & tensor,
      int out_w,
      int out_h,
      cv::Mat & depth_f32,
      cv::Mat & depth_gray,
  ...
  std::vector<Detection> decode_yolo_split_all_scores(const cv::Mat & boxes,
      const cv::Mat & all_scores,
      const cv::Mat & coeffs,
      int input_w,
      int input_h);
  std::vector<Detection> nms(std::vector<Detection> dets);
  void parse_yolo_outputs(const custom_msg::TensorList & tensors,
      int input_w,
      int input_h,
  ...
  // image msg builder
  sensor_msgs::msg::Image mat_to_image_msg(const cv::Mat & mat,
