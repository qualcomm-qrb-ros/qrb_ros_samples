...
static const char * COCO_CLASSES[] = { "person", "bicycle", "car", "motorcycle", "airplane", "bus",
...
static const cv::Scalar SEG_COLORS[] = { { 56, 56, 255 }, { 151, 157, 255 }, { 31, 112, 255 },
  { 29, 178, 255 }, { 49, 210, 207 }, { 10, 249, 72 }, { 23, 204, 146 }, { 134, 219, 61 },
  { 52, 147, 26 }, { 187, 212, 0 }, { 168, 153, 44 }, { 255, 194, 0 }, { 147, 69, 52 },
  { 255, 115, 100 }, { 236, 24, 0 }, { 255, 56, 132 }, { 133, 0, 82 }, { 255, 56, 203 },
  ...
  : Node("midas_yolo_fusion_node", options), last_log_time_(this->now())
{
  ...
  midas_in_pub_ =
      create_publisher<custom_msg::TensorList>("midas_inference_input_tensor", 10, ipc_opts);
  yolo_in_pub_ =
      create_publisher<custom_msg::TensorList>("yolo_seg_inference_input_tensor", 10, ipc_opts);
  ...
  image_sub_ = create_subscription<sensor_msgs::msg::Image>(input_topic_, 10,
  ...
  midas_out_sub_ = create_subscription<custom_msg::TensorList>(
      ...
      [this](custom_msg::TensorList::ConstSharedPtr msg) { midas_output_callback(msg); }, out_opts);

  yolo_out_sub_ = create_subscription<custom_msg::TensorList>(
      "yolo_seg_inference_output_tensor", 10,
      [this](custom_msg::TensorList::ConstSharedPtr msg) { yolo_output_callback(msg); }, out_opts);
...
cv::Mat MidasYoloFusionNode::decode_image(sensor_msgs::msg::Image::ConstSharedPtr msg)
{
  ...
  if (enc == "bgr8") {
    return cv::Mat(msg->height, msg->width, CV_8UC3, const_cast<uint8_t *>(msg->data.data()))
        .clone();
  }
  if (enc == "rgb8") {
  ...
  if (enc == "nv12") {
    cv::Mat yuv(msg->height * 3 / 2, msg->width, CV_8UC1, const_cast<uint8_t *>(msg->data.data()));
    cv::Mat bgr;
...
custom_msg::TensorList MidasYoloFusionNode::make_tensor_msg(const std::string & name,
    const cv::Mat & data,
    int data_type,
    ...
    const std::vector<uint32_t> & explicit_shape)
{
  ...
  if (!explicit_shape.empty()) {
    ...
    t.shape = { 1, static_cast<uint32_t>(data.rows), static_cast<uint32_t>(data.cols),
  ...
  if (++img_count % 20 == 0) {
    ...
    if (ms(now - last_img_log).count() > 2000) {
      RCLCPP_INFO(get_logger(),
          ...
          ms(t_decode - t_start).count(), ms(t_prep - t_decode).count(),
          ms(t_make - t_prep).count(), ms(t_pub - t_make).count(), ms(t_pub - t_start).count(),
          pending_.size());
      last_img_log = now;
    }
  }
...
cv::Mat MidasYoloFusionNode::tensor_to_mat(const custom_msg::Tensor & tensor)
{
  ...
  if (tensor.dmabuf_fd >= 0 && tensor.dmabuf_size > 0) {
    // mmap the ION buffer
    void * ptr = ::mmap(nullptr, tensor.dmabuf_size, PROT_READ, MAP_SHARED, tensor.dmabuf_fd,
        static_cast<off_t>(tensor.dmabuf_offset));
    if (ptr != MAP_FAILED) {
  ...
  if (byte_count == elem_count)
    cv_type = CV_8U;
  else if (byte_count == elem_count * 2)
    cv_type = CV_16U;
  else if (byte_count == elem_count * 4)
    cv_type = CV_32F;
  else if (byte_count == elem_count * 8)
    cv_type = CV_64F;
...
void MidasYoloFusionNode::decode_midas_depth(const custom_msg::Tensor & tensor,
    int out_w,
    int out_h,
    cv::Mat & depth_f32,
    cv::Mat & depth_gray,
    cv::Mat & depth_color)
{
  ...
  for (auto d : shape)
    if (d > 1)
      non1.push_back(static_cast<int>(d));
  if (non1.size() >= 2) {
    h = non1[non1.size() - 2];
    w = non1[non1.size() - 1];
  } else if (non1.size() == 1) {
  ...
  if (depth2d.cols != w) {
    RCLCPP_WARN(
        get_logger(), "decode_midas_depth: shape mismatch cols=%d expected w=%d", depth2d.cols, w);
  }
  if (depth2d.type() != CV_32F)
    depth2d.convertTo(depth2d, CV_32F);
...
std::vector<Detection> MidasYoloFusionNode::decode_yolo_split_all_scores(const cv::Mat & boxes_flat,
    const cv::Mat & all_scores_flat,
    ...
    int input_h)
{
  ...
  cv::Mat boxes_f32;
  if (boxes_flat.type() != CV_32F)
    boxes_flat.convertTo(boxes_f32, CV_32F);
  else
    boxes_f32 = boxes_flat;

  cv::Mat coeffs_f32;
  if (coeffs_flat.type() != CV_32F)
    coeffs_flat.convertTo(coeffs_f32, CV_32F);
  else
    coeffs_f32 = coeffs_flat;
  ...
  if (bt.cols != 4 || mt.cols != 32 || st.cols != NUM_COCO_CLASSES) {
    RCLCPP_WARN(
        get_logger(), "decode_yolo_split_all_scores: shape mismatch after reshape/transpose");
    return {};
  }
...
void MidasYoloFusionNode::parse_yolo_outputs(const custom_msg::TensorList & tensors,
    int input_w,
    int input_h,
    ...
    cv::Mat & proto_out)
{
  ...
  std::call_once(shape_dump_flag, [&]() {
    ...
    for (const auto & info : infos) {
      ...
      sh_str += "]";
      RCLCPP_INFO(get_logger(), "  name='%s' shape=%s elems=%zu", info.name.c_str(), sh_str.c_str(),
          info.flat.total());
    }
  });
  ...
  if (proto_flat.empty()) {
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
        "parse_yolo_outputs: proto tensor not found in %zu tensors", tensors.tensor_list.size());
    return;
  }
  if (boxes_flat.empty() || all_scores_flat.empty() || coeffs_flat.empty()) {
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
        "parse_yolo_outputs: missing tensors — boxes=%s scores=%s coeffs=%s",
        boxes_flat.empty() ? "missing" : "ok", all_scores_flat.empty() ? "missing" : "ok",
  ...
  dets_out =
      decode_yolo_split_all_scores(boxes_flat, all_scores_flat, coeffs_flat, input_w, input_h);
}
  ...
  for (int r = 0; r < depth.rows; ++r) {
    for (int c = 0; c < depth.cols; ++c) {
      if (mask.at<uint8_t>(r, c) > 0)
        vals.push_back(depth.at<float>(r, c));
    }
  }
  if (vals.empty())
    return 0.0f;
...
void MidasYoloFusionNode::draw_overlay(cv::Mat & out,
    const cv::Mat & depth_f32,
    const std::vector<Detection> & dets,
    const cv::Mat & proto_hwc,
    int input_w,
    int input_h)
{
  if (proto_hwc.empty() || dets.empty())
    return;
  ...
  for (const auto & det : dets) {
    ...
    // Label
    const char * cls_name =
        (det.cls >= 0 && det.cls < NUM_COCO_CLASSES) ? COCO_CLASSES[det.cls] : "unknown";
    ...
    char label[128];
    std::snprintf(label, sizeof(label), "%s %.0f%% d=%.2f", cls_name, det.score * 100.0f, dnorm);
    ...
    cv::rectangle(out, { txt_x, by1 },
        { std::min(out.cols - 1, txt_x + txt_sz.width + 2), txt_y + baseline }, cv_color,
        cv::FILLED);
    cv::putText(out, label, { txt_x + 1, txt_y - 1 }, cv::FONT_HERSHEY_SIMPLEX, 0.5, { 0, 0, 0 }, 1,
        cv::LINE_AA);
  }
}
...
sensor_msgs::msg::Image MidasYoloFusionNode::mat_to_image_msg(const cv::Mat & mat,
...
void MidasYoloFusionNode::fuse_and_publish(FrameKey key)
{
  ...
  if (!frame.midas_tensors || !frame.yolo_tensors)
    return;
  if (frame.midas_tensors->tensor_list.empty() || frame.yolo_tensors->tensor_list.empty())
    return;
  ...
  try {
    ...
    decode_midas_depth(frame.midas_tensors->tensor_list[0], frame.image_bgr.cols,
        frame.image_bgr.rows, depth_f32, depth_gray, depth_color);
    std::vector<Detection> dets;
    cv::Mat proto_hwc;
    parse_yolo_outputs(
        *frame.yolo_tensors, frame.yolo_input_w, frame.yolo_input_h, dets, proto_hwc);
    auto t1 = clk::now();

    // Draw overlay on depth_color
    cv::Mat overlay = depth_color.clone();
    draw_overlay(overlay, depth_f32, dets, proto_hwc, frame.yolo_input_w, frame.yolo_input_h);
    auto t2 = clk::now();
    ...
    if ((now - last_log_time_).seconds() > 2.0) {
      RCLCPP_INFO(get_logger(),
          "processed=%lu pending=%zu dets=%zu | decode+parse=%.2fms overlay=%.2fms pub=%.2fms "
          "total=%.2fms",
          cnt, pending_.size(), dets.size(), ms(t1 - t0).count(), ms(t2 - t1).count(),
          ms(t3 - t2).count(), ms(t3 - t0).count());
      last_log_time_ = now;
    }
