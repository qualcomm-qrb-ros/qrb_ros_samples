...
def main(args=None):
    # texts used for tests
    search_query = ["one black dog"]
    prepare_clip_text_tokenizer()
    txt_features = extract_text_feature(search_query)
    print(f"main : txt_features extracted : shape is {txt_features.shape}")
    
    # JPGs could all the jpg file path
    jpgs = ["1.jpg","2.jpg"]
    res = []
    for jpg in jpgs:
    ...
    print(f"main : images_features extracted : shape is {images_features.shape}")
    ...
    s,p,o = text_to_images_search_1N512(images_features, txt_features)
