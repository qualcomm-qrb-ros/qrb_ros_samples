...
class PoseEstimationNodeYcb(Node):
    def __init__(self):
        ...
        self.use_icp_refine = bool(self.get_parameter("use_icp_refine").value)
        ...
        self.refiner = TemplateICPPoseRefiner(PipelineConfig()) if self.use_icp_refine else None
    ...
    def _on_sync(self, *msgs):
        ...
        icp_pose = init_pose.copy()  # fallback: identity refinement
        ...
        if self.use_icp_refine:
            try:
                ...
                if len(points_icp) >= 50:
                    ...
                    # Update template using only ICP fitness gating (no DenseFusion confidence gate here).
                    self.refiner.update_template(points_icp, icp_pose, float(fitness))
        ...
        if self.use_icp_refine:
            ...
            self._publish_pose(rgb_msg, quat, trans, original_rot)
        ...
        self.get_logger().info(
            ...
            f"use_icp_refine={self.use_icp_refine} "
        ...
        vis_rot = refined_rot if self.use_icp_refine else original_rot
        vis_trans = refined_trans if self.use_icp_refine else trans
        self._save_visualization(rgb_msg, rgb, mask, vis_rot, vis_trans, conf, other_instances)
