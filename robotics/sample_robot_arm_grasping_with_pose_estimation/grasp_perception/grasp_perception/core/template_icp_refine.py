...
# ═══════════════════════════════════════════════════════════════════════════
# 1. Point Cloud Utilities (pure NumPy)
# ═══════════════════════════════════════════════════════════════════════════
...
class PoseEvaluator:
    """6D pose accuracy evaluation (optional module, requires GT)"""
...
# ═══════════════════════════════════════════════════════════════════════════
# 6. Pose Fusion (PnP + ICP weighted fusion)
# ═══════════════════════════════════════════════════════════════════════════

class PoseFusion:
    """Fuse PnP (strong rotation) and ICP (strong translation) results"""
    ...
    def fuse(
        ...
        """
        Fusion strategy:
          - translation: prefer ICP (geometric alignment is more accurate)
          - rotation: use PnP when successful with high inlier ratio, otherwise use ICP
          - fall back to DenseFusion when ICP fails
        ...
        # translation: always use ICP
        t_fused = icp_pose[:3, 3]

        # rotation: prefer PnP (if successful and high quality)
        chosen_rot = "icp"
...
# ═══════════════════════════════════════════════════════════════════════════
# 7. Main Pipeline (integrates all modules)
# ═══════════════════════════════════════════════════════════════════════════

class PoseRefinementPipeline:
    """
    ┌──────────────┐
    │  DenseFusion  │  initial pose
    └──────┬───────┘
           ▼
    ┌──────────────┐   ┌──────────────┐
    │ Template ICP  │   │  PnP Refiner  │   parallel
    ...
    def process_frame(
        ...
        Args:
            color_bgr:        Orbbec D335 BGR image
            depth:            Orbbec D335 depth map (uint16, mm)
            yolo_mask:        object mask from YOLO11 Seg output
            densefusion_pose: DenseFusion initial pose [4x4]
            gt_pose:          ground truth pose (optional)
        ...
        # ════════════════════════════════════════
        # first frame: initialize all modules
        # ════════════════════════════════════════
    ...
    def reset(self):
        """Reset all state (call when object is lost or re-detected)"""
        self.icp_refiner = TemplateICPRefiner(self.cfg)
        self.pnp_refiner = PnPRefiner(self.cfg)
        self.smoother.reset()
        self.frame_count = 0
...
if __name__ == "__main__":
    ...
    # ── configuration (adjust for your Orbbec D335) ──
    cfg = PipelineConfig(
