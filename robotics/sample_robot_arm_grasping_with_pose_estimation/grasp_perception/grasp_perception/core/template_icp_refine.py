...
Pipeline:
  ...
  RGB-D → Object Point Cloud + Feature Extraction
       │
  ┌────┴─────────────────────────┐
  │  PnP Refiner (rotation)      │  ← 标签纹理特征匹配
  │  Template ICP (translation)  │  ← 点云几何对齐
  └────┬─────────────────────────┘
       │
  Pose Fusion (加权融合)
       │
...
class RefinementResult:
    """单帧精化结果"""
    pose: np.ndarray                    # [4x4] 最终 pose
    R: np.ndarray = None                # [3x3] 旋转矩阵
    t: np.ndarray = None                # [3]   平移向量
...
class PipelineConfig:
    ...
    depth_max: float = 1.0          # 最大有效深度 (m)

    # ICP 参数
    icp_voxel_size: float = 0.002   # 体素降采样 (m)
    icp_max_dist: float = 0.015     # 最大对应距离 (m)
    icp_max_iter: int = 50
    ...
    ransac_reproj: float = 3.0      # RANSAC 重投影阈值 (px)
    ransac_iter: int = 200
    ransac_conf: float = 0.99

    # Mask 参数
    mask_dilate_px: int = 3         # mask 膨胀像素
    ...
    # 融合参数
    pnp_max_weight: float = 0.7     # PnP 旋转最大权重
    icp_fallback_fitness: float = 0.3  # ICP 回退阈值

    # 模板更新
    template_update: bool = True
    template_novel_dist: float = 0.004  # 新点判定距离 (m)
...
class PointCloudUtils:
    ...
    def prepare_mask(
        ...
        """YOLO bool mask → OpenCV uint8 mask (含膨胀)"""
...
# ═══════════════════════════════════════════════════════════════════════════
# 2. Template ICP Refiner (Point-to-Plane, 多尺度)
# ═══════════════════════════════════════════════════════════════════════════

class TemplateICPRefiner:
    """
    基于首帧模板的 Point-to-Plane ICP 精化。
    无 CAD 模型，无几何约束。
    """
    ...
    def init_template(
        ...
        self.template_normals = self.pcu.estimate_normals(pts_model)
    ...
    def update_template(
        ...
        if novel.sum() > 0:
            ...
            self.template_normals = self.pcu.estimate_normals(self.template_pts)
    ...
    def refine(
        ...
        for scale, max_d, max_it in zip(
            ...
            tgt_normals = self.pcu.estimate_normals(tgt_down)
...
class PnPRefiner:
    """
    基于特征匹配 + PnP 的 Pose 精化。
    用首帧 YOLO mask 内的特征 + 深度反投影建立 3D 数据库。
    """
    ...
    def refine(
        ...
        """
        当前帧: 特征匹配 → PnP + RANSAC → 精化 pose

        Returns:
            pnp_pose: [4x4] 或 None (失败时)
            info: 诊断信息
        """
        info = {
            "success": False,
...
# ═══════════════════════════════════════════════════════════════════════════
# 4. Temporal Pose Smoother (四元数加权平均)
# ═══════════════════════════════════════════════════════════════════════════
...
class PoseRefinementPipeline:
    ...
    def process_frame(
        ...
        gt_pose: np.ndarray = None,    # [4x4] (可选, 用于评估)
        verbose: bool = False,
    ) -> RefinementResult:
        """
        处理单帧。
