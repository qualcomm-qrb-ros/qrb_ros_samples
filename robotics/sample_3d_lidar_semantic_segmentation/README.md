...
```bash
sudo apt install ros-jazzy-sample-3d-lidar-semantic-segmentation
```
...
2. Prepare your x86 host and convert the model for HTP acceleration.
...
3. Cross-compiling QNN model shared library
...
Publish the 3D lidar pointCloud.
...
Open a new termial and run the sample on device
...
Open a new terminal and use RViz2 to visualize topic `/semantic_points`.
```
source /opt/ros/jazzy/setup.bash
export ROS_DOMAIN_ID=222
rviz2
...
```bash
cd ~/qrb_ros_sample_ws/src/qrb_ros_samples/ai_robotics/sample_3d_lidar_semantic_segmentation
